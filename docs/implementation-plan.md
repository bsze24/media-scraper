...
| 28 | **Lost corrections silent on reprocess.** `mergeCorrectedTurns()` (`lib/queue/orchestrator.ts:51`) preserves human-corrected turns during reprocess by matching `turn_index`, but when a correction's `turn_index` doesn't exist in the new turn set (e.g., re-scrape changed turn boundaries), the loss is only a `console.log` (line 79). Manual edits are high-effort — silently dropping them is unacceptable. Fix: (a) persist lost correction count + details to `processing_warnings` so it surfaces in admin, (b) add a confirmation step or dry-run mode for reprocess that shows "N corrections will be lost" before proceeding, (c) consider storing corrections independently (keyed by text hash or timestamp range) so they survive turn re-indexing. | P1 | Before reprocess is used on appearances with manual corrections. |
